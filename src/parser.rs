...
fn parse_fields(buffer: &mut Vec<u8>, length: u16, cp: &ConstantPool) -> Result<Vec<Field>, ParserError> {
...
fn parse_field(buffer: &mut Vec<u8>, cp: &ConstantPool) -> Result<Field, ParserError> {
    let access_flags = parse_u16(buffer)?;
    let name_index = parse_u16(buffer)?;
    let descriptor_index = parse_u16(buffer)?;
    let attributes_count = parse_u16(buffer)?;
...
fn parse_methods(buffer: &mut Vec<u8>, length: u16, cp: &ConstantPool) -> Result<Vec<Method>, ParserError> {
...
fn parse_method(buffer: &mut Vec<u8>, cp: &ConstantPool) -> Result<Method, ParserError> {
    let access_flags = parse_u16(buffer)?;
    let name_index = parse_u16(buffer)?;
    let descriptor_index = parse_u16(buffer)?;
    let attributes_count = parse_u16(buffer)?;
...
fn parse_attributes(buffer: &mut Vec<u8>, length: u16, cp: &ConstantPool) -> Result<Vec<Attribute>, ParserError> {
...
fn parse_attribute(buffer: &mut Vec<u8>, cp: &ConstantPool) -> Result<Attribute, ParserError> {
